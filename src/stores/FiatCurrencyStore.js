...
class FiatCurrencyStore {
  ...
  @action.bound async getSpotRate(base, quote) {
    if (base === quote) return 1;
    const SUPPORTED_TSYM_CURRENCIES = ['BTC', 'USDT', 'F:USD', 'ETH'];
    const isSupportedCurrency = SUPPORTED_TSYM_CURRENCIES.includes(quote);
    let mRate = 0;
    if (isSupportedCurrency) {
      mRate = await this.getSpotPrice(base, quote);
    }
    if (!isSupportedCurrency || mRate === 0) {
      const c1Rate = await this.getSpotPrice(base, 'BTC');
      const c2Rate = await this.getSpotPrice(quote, 'BTC');
      mRate = c1Rate / c2Rate;
    }
    return mRate;
  }

  getSpotPrice(base, quote) {
    return new Promise(resolve => {
