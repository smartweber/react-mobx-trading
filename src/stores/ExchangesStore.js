...
class ExchangesStore {
  ...
  constructor(instrumentStore, settingsStore, snackbar, smsAuthStore) {
  ...
  @action.bound setExchangeActive(key, value) {
    if (key === 'Global') {
      if (this.exchanges.Global && this.exchanges.Global.active && !value) {
        // do nothing if Global already selected
        return;
      }
...
export default (instrumentStore, settingsStore, snackbar, smsAuthStore) => {
  const store = new ExchangesStore(instrumentStore, settingsStore, snackbar, smsAuthStore);
  return store;
};
