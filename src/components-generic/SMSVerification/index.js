...
const injectProps = compose(
  ...
  withProps(
    ({
      [STORE_KEYS.SMSAUTHSTORE]: { requestAuthCode, confirmAuthCode },
      [STORE_KEYS.TELEGRAMSTORE]: { initByTelegramLogin, loginFinishWith },
      [STORE_KEYS.ORDERHISTORY]: { requestOrderHistory },
      [STORE_KEYS.YOURACCOUNTSTORE]: { requestPositionWithReply },
      [STORE_KEYS.LOWESTEXCHANGESTORE]: { requestOrderEvents }
    }) => ({
      requestAuthCode,
      confirmAuthCode,
      initByTelegramLogin,
...
class SMSVerification extends Component {
  ...
  handleSendPhoneNumber = e => {
    ...
    const { requestAuthCode } = this.props;
  ...
  handleCheckCode = () => {
    ...
    const { confirmAuthCode } = this.props;
...
export default injectProps(SMSVerification);
