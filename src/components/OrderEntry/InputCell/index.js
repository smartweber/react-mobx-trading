...
import { Wrapper, Content, OrderSlider, Symbol, AutoPos } from './styles';
...
export default class MarketInputCell extends Component {
  ...
  handleSliderChange = (obj, val) => {
    const { type, setEndTouch } = this.props;
    const isDisabled = type !== 'buy_from' && type !== 'sell_from';
    if (!isDisabled) {
      const value = convertToFloat(val / MULTIPLIER);
      const isEndTouch = value === this.props.max;

      if (setEndTouch) setEndTouch(isEndTouch);
  ...
  render() {
     ...
     */
    const { value, symbol, isBuy, max, type, width, height, isLeft, isHover, animation, originUSDPos } = this.props;
    const isDisabled = (type !== 'buy_from' && type !== 'sell_from') || !Number(max.toFixed(6));
    const isUSD = symbol === 'USD';
    let valueOfAmount = isUSD ? customDigitFormatWithDecimals(value, 2) : customDigitFormat(value, 6);
    if (valueOfAmount === '') {
      valueOfAmount = '0';
    }
    ...
    return (
      ...
      >
        <Content width={width} height={height}>
          ...
          {type === 'buy_from' && originUSDPos > 0 && value > 0.0001 && <AutoPos position={originUSDPos} />}
        </Content>
      </Wrapper>
    );
  }
