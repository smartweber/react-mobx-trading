...
import { Label, GlobalIcon, Logo } from './styles';
    ...
    <Label id={id} className={className} insideGraph={insideGraph} isOnTop={isOnTop} isRight={isRight}>
      {!arbMode && !isTradingView && (
        <Label>
          ...
          {activeExchangesText ? (
            <span>{`${activeExchangesText} (${selectedBaseSymbol}/${selectedQuoteSymbol})`}</span>
          ) : (
            <span>{`${selectedBaseSymbol}/${selectedQuoteSymbol}`}</span>
          )}
        </Label>
      )}
...
export default compose(
  ...
  withProps(
    ({
      [STORE_KEYS.EXCHANGESSTORE]: { exchanges, marketExchanges, activeExchangesText },
