...
class ExchangeList extends React.Component {
  ...
  getApiDocument = () => {
    const pageName = this.state.selectedExchange.name.toLowerCase().replace(/[\. ,:-]+/g, '');
  ...
  itemCellRenderer = ({ rowData, rowIndex }) => {
    ...
    const { hoveredIndex, selectedExchange, selectedMenu, isOpenAPIDocs } = this.state;
    let isActive = false;
    ...
    return (
      ...
      >
        ...
        {activeExchange && setExchangeActive && !isOpenAPIDocs && (
          <div
            className="api-link"
            role="button"
            tabIndex={0}
