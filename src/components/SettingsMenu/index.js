...
class SettingsMenu extends React.Component {
  ...
  toggleGoogle2FAModal = () => {
    this.setState(prevState => ({
      isGoogle2FAModalOpen: !prevState.isGoogle2FAModalOpen
    }));
  };

  openGoogle2FA = () => {
    this.setState({ isGoogle2FAModalOpen: true });
  };

  toggleEmail2FAModal = () => {
    this.setState(prevState => ({
      isEmail2FAModalOpen: !prevState.isEmail2FAModalOpen
    }));
  };

  openEmail2FA = () => {
    this.setState({ isEmail2FAModalOpen: true });
  };
  ...
  handleModeSwitch = (isDeposit = true) => {
    ...
    return (
      <CollapseComponent expanded={this.state.openedSettings === SETTINGS_ITEMS.PRIVACY}>
        <MenuItem
          ...
          onClick={!isGet2FA && this.openGoogle2FA}
        >
          {isGet2FA ? (
            ...
            <SwitchItem isChecked={isGoogle2FA} handleChange={this.openGoogle2FA} />
          )}
        </MenuItem>
        <MenuItem
          ...
          onClick={!isGet2FA && this.openEmail2FA}
        >
          {isGet2FA ? (
            <div className="spinner-wrapper">
              <Spinner />
            ...
            <SwitchItem isChecked={isEmail2FA} handleChange={this.openEmail2FA} />
          )}
        </MenuItem>
      </CollapseComponent>
    );
  ...
  renderDepositWithdrawContent = () => {
    ...
    const placeholder = isDeposit ? 'Any Bitcoin' : '0.0007 (for gas)';
    const subTitle = isDeposit ? (
      <div className="deposit-sub-title">
        <span>Your wallet address will be auto filled after deposit.</span>
        <span>You may edit this after it is filled.</span>
      </div>
    ) : (
      '0.0007 BTC will then be withdrawn from the ledger and sent to your wallet'
    );

    return (
      <div className="content">
        <div className="title">Send from your wallet</div>
        <div className="input">
          <div className="coin-icon">
            <CoinIcon fontIcon value="BTC" size={27} />
          </div>
          <ModeInputWrapper placeholder={placeholder} />
        </div>
  ...
  renderModeMenuItem = () => {
    const { isRealTrading, coinDepositAddress } = this.props;
    const { isCopied, isDeposit } = this.state;
    return (
        ...
        <CollapseComponent expanded={isRealTrading && coinDepositAddress}>
          <TransferSection>
            <ModeMainWrapper>
              <WithdrawDepositWrapper>
                <ModeSwitchWrapper>
                  <DepositButton onClick={() => this.handleModeSwitch(true)} isActive={isDeposit}>
                    Deposit
                    {isDeposit && (
                      <CaretArrowIcon borderColor="#04c6c0" className="dropdown_arrow" fillColor="#04c6c0" />
                    )}
                  </DepositButton>
                  <WithdrawButton onClick={() => this.handleModeSwitch(false)} isActive={!isDeposit}>
                    Withdraw
                    {!isDeposit && (
                      <CaretArrowIcon borderColor="#04c6c0" className="dropdown_arrow" fillColor="#04c6c0" />
                    )}
                ...
                <ModeContentWrapper>{this.renderDepositWithdrawContent()}</ModeContentWrapper>
              </WithdrawDepositWrapper>
              <DepositQRCodeWrapper>
                <DepositQRCode>
                ...
                <CopyButton onClick={this.copyToClipboard} isCopied={isCopied}>
                  Copy To Clipboard
                  <div className="tippy-tooltip bct-theme">Copied.</div>
                </CopyButton>
              </DepositQRCodeWrapper>
            </ModeMainWrapper>
          </TransferSection>
    ...
    return (
      <Wrapper>
        <MenuWrapper>
          ...
          <Footer>
            <FooterButtonsWrapper>
              <FooterMenuItem onClick={this.handlePageOpen('terms-of-use')}>
                <FormattedMessage id="settings.button_terms_of_use" defaultMessage="Terms of Use" />
              </FooterMenuItem>
              <FooterMenuItem onClick={this.handlePageOpen('privacy-policy')}>
                <FormattedMessage id="settings.button_privacy_policy" defaultMessage="Privacy Policy" />
              </FooterMenuItem>
              <FooterMenuItem onClick={this.handleHelpdeskOpen}>
                <FormattedMessage id="settings.button_support_center" defaultMessage="Helpdesk" />
              </FooterMenuItem>
            </FooterButtonsWrapper>
            <FooterButtonsWrapper>
              {!isRealTrading && (
                <FooterMenuItem onClick={this.handleResetBalance}>
                  <FormattedMessage id="settings.button_reset" defaultMessage="Reset" />
                </FooterMenuItem>
              )}
              <FooterMenuItem onClick={this.toggleLogoutModal}>
                <FormattedMessage id="settings.button_logout" defaultMessage="Logout" />
              </FooterMenuItem>
