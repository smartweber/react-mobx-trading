...
import { Container, Wrapper, CoinName, CoinIconWrapper } from './styles';
  ...
  ({ children, type, cellWidth, coin, isHovered, showArrow, showCoinName, ...formatDigitProps }) => {
    ...
    return (
      <Wrapper cellWidth={cellWidth} isHovered={isHovered} type={type}>
        {isHovered &&
          (showArrow ? (
            <ArrowComponent className="arrow-icon" size={14} />
          ) : (
            <CoinIcon filter={COIN_COLORS[type]} fontIcon size={12} value={coin} />
          ))}
