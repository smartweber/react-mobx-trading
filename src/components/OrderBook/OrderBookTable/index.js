...
import { getNumberParts } from '@/utils';
...
class OrderBookTable extends PureComponent {
  ...
  onRowClick = data => () => {
    // TODO: set values in OrderForm
    console.log(`amount: ${data.amount}, price: ${data.price}`);
  };
  ...
  getRows = (type, items, cellWidth, numberFormatMeta) => {
    const { hoveredItem = {}, price: globalPrice } = this.props;
    ...
    return items.map(({ price, amount, cost, exchange, total, cumulativeAmount }, index) => {
      ...
      return (
        <Row
          key={index}
          onClick={this.onRowClick({ amount, price })}
  ...
  getHeader = (cellWidth, numberFormatMeta) => {
    ...
    return (
      <HeaderRow>
        <ExchangeHeader text={<ExchangesLabel />} onClick={setUserDropDownOpen} cellWidth={exchangeWidth} />
        <TotalAmountHeader
          coin={baseSymbol}
