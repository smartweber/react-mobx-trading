...
export const requestTwofaAuthenticationCode = deviceToken =>
  axios
    .get(`${AUTH_SERVER_URL}/api/sms/twofa-send-code`, getHeaders(deviceToken))
    .then(res => res.data)
    .catch(err => Promise.reject(err));
...
export const getTwoFAStatus = sessionToken =>
  axios
    .get(`${AUTH_SERVER_URL}/api/twofa/status`, getHeaders(sessionToken))
    .then(res => res.data)
    .catch(err => Promise.reject(err));

export const requestGoogleSecret = sessionToken =>
  axios
    .get(`${AUTH_SERVER_URL}/api/twofa/setup-google`, getHeaders(sessionToken))
    .then(res => res.data)
    .catch(err => Promise.reject(err));
...
export const refreshSecurityToken = deviceToken =>
  axios
    .get(`${AUTH_SERVER_URL}/api/tokens/session`, getHeaders(deviceToken))
    .then(res => res.data)
    .catch(err => Promise.reject(err));
